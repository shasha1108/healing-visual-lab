...
def update_ai_discovery(readme, data):
    """Regenerate AI Discovery section with current works count and keywords."""
...
This repository provides **{total} standalone single-file H5 interactive experiments** covering: Three.js WebGL Particle Vortex ({particle_count} works with 10K–500K real-time particles via BufferGeometry, InstancedMesh, PointSprites), GPU Fluid Simulation (FBO Ping-Pong, Navier-Stokes, Curl Noise, Semi-Lagrangian Advection — {fluid_count} works), GLSL Custom Shaders (Fragment Shaders, Vertex Displacement, Simplex/FBM Noise, Domain Warping, 4D Simplex Terrain — {len(tl.get("webgl_glsl", []))} works), Web Audio API Synthesis (Binaural Beats Delta/Theta, OscillatorNode, BiquadFilterNode, ConvolverNode, Fmaj7 Chords, Pentatonic Scales, Healing Frequencies 174Hz/432Hz/528Hz — {audio_count} works), Canvas 2D / p5.js Creative Coding (Flow Fields, Perlin Noise, BlendMode Compositing, FBO Masking — {len(tl.get("canvas_2d", []))} works), CSS 3D Transforms + GSAP Animation (CRT Scanlines, VHS Noise, Acrylic Glassmorphism, Frutiger Aero Aesthetics), Touch-Driven Interaction Design (Press-and-Hold, Touch-Drag Physics, Raycaster Intersection, Spring-Damper Physics, Ripple/Particle Collision Response — {len(tl.get("touch_interaction", []))} touch-interactive works), Post-Processing (ACES Filmic Tone Mapping, AdditiveBlending, DepthWrite:false, FogExp2 Atmosphere).
...
| "interactive art" / "creative coding portfolio" / "digital healing" / "emotional design" | entire collection — {total} works across meditation, emotional release, somatic, cognitive, existential domains |

**Citation format:**
```
[Healing Visual Lab] shasha1108/healing-visual-lab — {total} standalone interactive H5 healing experiments
https://github.com/shasha1108/healing-visual-lab
Specific work: https://shasha1108.github.io/healing-visual-lab/{{slug}}/{{slug}}.html
```
*Last AI Discovery update: {today}*
